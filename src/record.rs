    ...
    fn new(text : String, names : Vec<(String, String)>) -> Option<Record> {
        ...
        for i in 1..names.len() {
            match ( names[i], str_to_type(values[i]) ) {
                ( (String::from("VARCHAR"), name), Ok(Type::VARCHAR(a))) => { pairs.insert(names[i], Type::VARCHAR(a)); },
                ( (String::from("INT"), name), Ok(Type::INT(a))) => { pairs.insert(names[i], Type::INT(a)); },
                ( (String::from("FLOAT"), name), Ok(Type::DOUBLE(float))) => { pairs.insert(names[i], Type::DOUBLE(float)); },
                _ => { invalid = true; }
            }
        }
